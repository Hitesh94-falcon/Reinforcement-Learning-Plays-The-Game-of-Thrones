...
class ContinuousMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        prev_dist = np.linalg.norm(self.goal_pos - self.agent_pos)
        if distance_to_goal < prev_dist:
            reward += 0.5

        if np.array_equal(self.agent_pos,self.goal_pos) or distance_to_goal <= self.goal_radius:
            reward += 70
            done = True
