...
class PadmEnv(gym.Env):
    ...
    def reset(self):
        ...
        if self.random_initialization:
            ...
            if self.state == any(np.array_equal(self.agent_state, obs) for obs in self.obstacles):
    ...
    def step(self, action):
        ...
        in_obstacle = any(np.array_equal(self.agent_state, obs) for obs in self.obstacles)
        if in_obstacle:
            reward = -10
...
def create_env(steps = 1000,grid_size=10):
    env = PadmEnv(grid_size=grid_size)
    state = env.reset()
